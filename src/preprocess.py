...
class _CocoDataset(Dataset):
    def __init__(self, root: str, split: str, img_res: int, max_samples: int):
        ...
        if not ann_file.exists():
            raise FileNotFoundError(
                "COCO annotation file not found. Please download the dataset first."
            )
    ...
    def __getitem__(self, i):
        ...
        tokens = self.tok(
            sent, padding="max_length", max_length=77, return_tensors="pt", truncation=True
        )
        return img, tokens["input_ids"].squeeze(0)


def get_coco_loader(
    root: str, split: str, img_res: int, batch_size: int, max_samples: int
):
    """Return a DataLoader for the requested COCO split.  Unlike the random
    fallback loader, this helper returns *one* loader because train/val loaders
    are created separately in `train.py`.
    """
...
class _UCFClips(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frames = torch.nn.functional.interpolate(
            frames, size=self.res, mode="bilinear", align_corners=False
        )
        frames = frames * 2 - 1
